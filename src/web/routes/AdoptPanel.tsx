...
/**
 * Spec §8's "adopt from disk", extracted from `AdoptDialog` (Phase 1E) when Task 6 gave
 * the setup wizard's import step (`StepImport`) an identical adoption flow with
...
export function AdoptPanel({
   ...
   * invalidating the app list — the one moment either caller is allowed to move on.
   * `AdoptDialog` closes; `StepImport` advances the wizard. A response carrying any
   * `failed` entries never calls this: closing or advancing on a partial failure would
    ...
    <>
      ...
      <div className="mt-4 flex justify-end gap-2">
        ...
        <button
          type="button"
          onClick={handleAdopt}
          disabled={selected.size === 0 || submitting.size > 0 || disabled}
