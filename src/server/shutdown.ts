...
export type ShutdownOptions = {
   ...
   * are synchronous, `db.close()` is synchronous, and `jobs.shutdown()` already carries its
   * own internal timeout (`JobRunner`'s default is 10s). Bounding `server.close()` this way
   * means the sequence is *always* guaranteed to reach `db.close()` — never just "probably,
   * if nothing else also happens to hang" — which is the point: an unflushed SQLite handle
